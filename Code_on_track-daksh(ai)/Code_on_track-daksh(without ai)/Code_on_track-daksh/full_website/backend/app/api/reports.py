...
async def get_summary_report(
    ...
    # 8. Items with QR codes (Mocked as 100% since no column tracks it) or remove.
    # We will assume all items have QR for now as it's implied by UID presence.
    items_with_qr = total_items

    # 9. Warranty expiring in 30 days (check warranty_months from manufacture_date)
    warranty_expiring = 0
    try:
        warranty_result = await db.execute(
            select(func.count(Item.id)).where(
                Item.manufacture_date.isnot(None),
                Item.warranty_months.isnot(None),
                # warranty_months -> years roughly or just months addition
                # Postgres interval '1 month' * months
                # defaulting to simple days approx for compatibility if interval arithmetic is complex in ORM without specific dialect imports
                # assuming 30 days per month
                func.date(Item.manufacture_date) + (Item.warranty_months * 30) <= func.current_date() + 30,
                func.date(Item.manufacture_date) + (Item.warranty_months * 30) >= func.current_date()
            )
        )
